package framework;

public class Property {

    public static final int SKILL_ICON_SIZE = 45;
    public static final int SKILL_ICON_FILE_SIZE = 15;
    public static final int PORTRAIT_SIZE = 60;
    public static final int PORTRAIT_FILE_SIZE = 20;
    public static final int EFFECT_ICON_SIZE = 8;
    public static final int EFFECT_ICON_FILE_SIZE = 8;

    //TEXT
    public static final int BASE_LINE_DISTANCE = 5;
    public static final int BUTTON_NORMAL_HEIGHT = 24;
    public static final int BUTTON_NORMAL_WIDTH = 80;
    public static final int ABILITY_BUTTON_WIDTH = 150;
    public static int BUTTON_PADDING = 2;
    public static int BUTTON_LINE = 2;
    public static int BASE_CHAR_DISTANCE = 1;
    public static int BASE_TEXT_BLOCK_HEIGHT = 100;

    public static int ARENA_SPRITES_Y = 100;

    //HUD
    public static int ACTIVE_CHAR_OV_X = 10;
    public static int ACTIVE_CHAR_OV_Y = 230;
    public static int ACTIVE_CHAR_OV_WIDTH = 128;
    public static int ACTIVE_CHAR_OV_HEIGHT = 82;
    public static int TEAM_OVERVIEW_WIDTH = 828;
    public static int TEAM_OVERVIEW_HEIGHT = 120;
    public static int TEAM_OVERVIEW_X = 88;
    public static int TEAM_OVERVIEW_Y = 550;
    public static int ACTIVE_ABILITY_WIDTH = 240;
    public static int ACTIVE_ABILITY_HEIGHT = 92;
    public static int ACTIVE_ABILITY_X = 190;
    public static int ACTIVE_ABILITY_Y = 230;
    public static int LOG_WIDTH = 128 + 20; //leaving room for the button
    public static int LOG_HEIGHT = 96 + 20;
    public static int LOG_X = 482;
    public static int LOG_Y = 230 - 20;
    public static int LOG_ITEM_W = 128;
    public static int LOG_ITEM_H = 20;




    public static int TEAM_OVERVIEW_CONTENT_WIDTH = 828;
    public static int TEAM_OVERVIEW_CONTENT_HEIGHT = 300;
    public static int TEAM_OVERVIEW_CONTENT_X = 88;
    public static int TEAM_OVERVIEW_CONTENT_Y = 680;
}
