...
public class HUN_Single_out extends Skill {
    ...
    public Skill getCast() {
        HUN_Single_out cast = new HUN_Single_out(this.Hero);
        cast.copyFrom(this);
        return cast;
    }
    @Override
    ...
    public int getAccuracyFor(Skill s, int baseAcc){
        if (s.Hero == this.Hero && s.getTargetType().equals(TargetType.SINGLE)) {
            Logger.logLn(this.Hero.name + ".HUN_Single_out.getAccuracyFor");
